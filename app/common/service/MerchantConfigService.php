...
use app\common\model\MerchantConfig;
...
class MerchantConfigService
{
    ...
    public static function set(string $type, string $name, $value)
    {
        ...
        $data = MerchantConfig::where(['type' => $type, 'name' => $name])->findOrEmpty();
    ...
    {
        if (!empty($name)) {
            $value = MerchantConfig::where(['type' => $type, 'name' => $name])->value('value');
            if (!is_null($value)) {
        ...
        // 取某个类型下的所有name的值
        $data = MerchantConfig::where(['type' => $type])->column('value', 'name');
        foreach ($data as $k => $v) {
